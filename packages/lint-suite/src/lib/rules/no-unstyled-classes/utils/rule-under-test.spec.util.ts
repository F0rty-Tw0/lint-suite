...
import { angularTemplate } from '../../../angular-template.ts';
...
const plugins = angularTemplate.map((config) => config.plugins?.[pluginName]);
const plugin = plugins.find(Boolean);
const registeredRule = plugin?.rules?.['no-unstyled-classes'];
const parser = isCompatibleParser(templateParser) ? templateParser : null;

assert.ok(
  registeredRule,
  'angularTemplate preset must register no-unstyled-classes'
);
