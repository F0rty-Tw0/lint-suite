...
const rule = typescript.map((config) => config.plugins?.['local']).find(Boolean)
  ?.rules?.['ternary-branch-shape'];

assert.ok(rule, 'typescript preset must register local/ternary-branch-shape');
...
ruleTester.run('local/ternary-branch-shape', rule, { valid, invalid });
