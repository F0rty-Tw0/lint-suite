...
const rule = typescript.map((config) => config.plugins?.['local']).find(Boolean)
  ?.rules?.['chain-receiver-is-name'];

assert.ok(rule, 'typescript preset must register local/chain-receiver-is-name');
...
ruleTester.run('local/chain-receiver-is-name', rule, { valid, invalid });
