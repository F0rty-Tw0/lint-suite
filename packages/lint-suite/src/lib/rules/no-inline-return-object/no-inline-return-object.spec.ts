...
const rule = typescript.map((config) => config.plugins?.['local']).find(Boolean)
  ?.rules?.['no-inline-return-object'];

assert.ok(rule, 'typescript preset must register local/no-inline-return-object');
...
ruleTester.run('local/no-inline-return-object', rule, { valid, invalid });
