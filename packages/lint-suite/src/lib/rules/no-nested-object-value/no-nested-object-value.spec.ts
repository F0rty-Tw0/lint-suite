...
const rule = typescript.map((config) => config.plugins?.['local']).find(Boolean)
  ?.rules?.['no-nested-object-value'];

assert.ok(rule, 'typescript preset must register local/no-nested-object-value');
...
ruleTester.run('local/no-nested-object-value', rule, { valid, invalid });
