...
const rule = typescript.map((config) => config.plugins?.['local']).find(Boolean)
  ?.rules?.['no-spread-expression'];

assert.ok(rule, 'typescript preset must register local/no-spread-expression');
...
ruleTester.run('local/no-spread-expression', rule, { valid, invalid });
