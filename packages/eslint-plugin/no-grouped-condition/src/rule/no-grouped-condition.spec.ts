...
const rule = plugin.rules?.['no-grouped-condition'];
...
const groupedConditionError = (output: string): RuleTester.TestCaseError => {
...
const invalid: RuleTester.InvalidTestCase[] = [
  {
    ...
    code: `if (a && (b || c)) {}`,
    errors: [
      groupedConditionError(`const isGroup = b || c;\nif (a && (isGroup)) {}`)
    ]
  },
  {
...
ruleTester.run('no-grouped-condition/no-grouped-condition', rule, {
  valid,
  invalid
});
