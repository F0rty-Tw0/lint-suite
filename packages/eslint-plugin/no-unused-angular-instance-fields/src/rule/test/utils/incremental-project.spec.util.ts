...
const restorePackageManifest = (projectDirectory: string): void => {
  const fixtureManifest = join(projectDirectory, 'package.fixture.json');
  const packageManifest = join(projectDirectory, 'package.json');

  renameSync(fixtureManifest, packageManifest);
};
...
export const createIncrementalProject = (): IncrementalProject => {
  ...
  restorePackageManifest(projectDirectory);
